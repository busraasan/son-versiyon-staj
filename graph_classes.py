...
class ClientReport:
    ...
    def add_figure(self, path, pdf, width, is_last):
        ...
        if(self.figure_num % 2 == 0 or width > WIDTH/2-5):
            ...
            if(width == WIDTH):
                pdf.image(path, 5, 100*(self.chart_row-1)-self.minus+80, width)
                self.figure_num += 2
                self.figure_per_page_num +=2
            elif(width > WIDTH/2-5):
                pdf.image(path, 40, 100*self.chart_row-self.minus, width)
                self.figure_num += 2
                self.figure_per_page_num +=2
            else:
                pdf.image(path, 5, 100*self.chart_row-self.minus, width)
                self.figure_per_page_num += 1
                self.figure_num += 1
            #self.y = 80*chart_row-minus
            #self.x = WIDTH/2+5
        else:
            pdf.image(path, WIDTH/2+5, 100*self.chart_row-self.minus, width)
            self.figure_per_page_num +=1
            self.chart_row += 1
            self.figure_num += 1
    ...
    def usage_for_every_host(self, pdf):
        ...
        for host in self.client.hostnames:
            ...
            self.add_figure(path3, pdf, WIDTH, is_last)
...
if __name__ == "__main__":
    ...
    load_to_database("/Users/busraasan/Downloads/busra_cpu.xlsx", "basanto")
